...
const cars: Record<CarId, { name: string; label: string; color: string; speed: number; handling: number; body: string; strength: string; weakness: string }> = {
  ...
  レグルス: { name: "レグルス", label: "幻のレジェンドカー", color: "#c9d6e3", speed: 90, handling: 74, body: "legend", strength: "初級・中級・上級を制覇した者だけが出会える伝説の一台。速さも曲がりも一級品。", weakness: "長い眠りから覚めたばかり。乗りこなすには改造での目覚ましが必要。" },
};
...
export default function Home() {
  const [tier, setTier] = useState<CarId | null>(null); const [challengeTier, setChallengeTier] = useState<Tier>("初級"); const [quiz, setQuiz] = useState<Question[]>([]); const [i, setI] = useState(0); const [score, setScore] = useState(0); const [choice, setChoice] = useState<number | null>(null); const [parts, setParts] = useState({ tires:0, wing:0, ride:0, engine:0 }); const [tickets, setTickets] = useState(0); const [raceCount, setRaceCount] = useState(0); const [transferCode, setTransferCode] = useState(""); const [restoreCode, setRestoreCode] = useState(""); const [saveMessage, setSaveMessage] = useState(""); const [screen,setScreen] = useState<"welcome"|"select"|"garage"|"quiz"|"tune"|"race">("welcome"); const [clearedTiers, setClearedTiers] = useState<Set<Tier>>(new Set());
  const car = tier ? cars[tier] : null; const tokens = tickets; const stat = car ? {speed:car.speed+parts.engine*8+parts.tires*3+parts.wing*2, handling:car.handling+parts.tires*5+parts.ride*7+parts.wing*2} : {speed:0,handling:0}; const legendUnlocked = clearedTiers.size>=3;
  ...
  const next=()=>{ if(i===quiz.length-1){setTickets(t=>t+Math.floor(score/5));setClearedTiers(prev=>{const n=new Set(prev);n.add(challengeTier);return n});setScreen("tune");}else{setI(x=>x+1);setChoice(null)}};
  ...
  if(!tier) return <main className="app select-page"><header><p className="eyebrow">CAR SELECT</p><h1>まず車を<br/>選ぼう。</h1><p>車によって、得意なコースと苦手なことが違うよ。</p></header><section className="choose"><div className="car-grid">{(Object.keys(cars) as CarId[]).map(t=>{const locked=t==="レグルス"&&!legendUnlocked;return <button key={t} className={`car-card detailed${locked?" locked":""}`} style={{"--car":cars[t].color} as React.CSSProperties} disabled={locked} onClick={()=>{if(locked)return;setTier(t);setChallengeTier("初級");setParts({tires:0,wing:0,ride:0,engine:0});setTickets(0);setRaceCount(0);setTransferCode("");setSaveMessage("");setScreen("garage")}}><div className={`car ${cars[t].body}${locked?" silhouette":""}`} style={{"--car":cars[t].color} as React.CSSProperties}><i/><b/></div><strong>{locked?"？？？":cars[t].name}</strong><small>{locked?"全クラス制覇で解放":cars[t].label}</small><p className="strength">{locked?"初級・中級・上級、すべてのクラスをクリアすると挑戦できる幻の一台。":`得意：${cars[t].strength}`}</p>{!locked&&<p className="weakness">注意：{cars[t].weakness}</p>}<em>{locked?"クラス制覇まであと少し":"この車を選ぶ →"}</em></button>})}</div></section><section className="transfer"><h2>続きからあそぶ</h2><p>前回もらった10文字の引き継ぎコードを入力してください。</p><div><input aria-label="引き継ぎコード" maxLength={10} value={restoreCode} onChange={e=>setRestoreCode(e.target.value.toUpperCase())} placeholder="例：AB2CDE3FGH"/><button className="secondary" onClick={restoreGarage}>引き継ぐ</button></div>{saveMessage&&<p className="save-message">{saveMessage}</p>}</section></main>;
