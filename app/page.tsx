...
export default function Home() {
  ...
  if(!tier) return <main className="app"><header><p className="eyebrow">お店で遊べる！</p><h1>ピットイン<br/><span>クイズレーサー</span></h1><p>クイズに正解して、車を育て、<br/>レースを勝ち抜こう。<br/><b>あなたはどこまで行ける？</b></p></header><section className="choose"><h2>相棒の車をえらぼう</h2><div className="car-grid">{(Object.keys(cars) as Tier[]).map(t=><button key={t} className="car-card" style={{"--car":cars[t].color} as React.CSSProperties} onClick={()=>{setTier(t);setChallengeTier("初級");setParts({tires:0,wing:0,ride:0,engine:0});setTickets(0);setRaceCount(0)}}><div className={`car ${cars[t].body}`}><i/><b/></div><strong>{cars[t].name}</strong><small>{cars[t].label}</small><em>{t}</em></button>)}</div></section><p className="foot">車の種類に関係なく、すべてのクラスに挑戦できます。</p></main>;
