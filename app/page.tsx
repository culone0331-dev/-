...
const cars = {
  初級: { name: "ポコ", label: "軽四ビギナー", color: "#47d7a3", speed: 54, handling: 78, body: "compact" },
  中級: { name: "ノア", label: "小型・普通車チャレンジャー", color: "#4eb3ff", speed: 70, handling: 66, body: "sedan" },
  上級: { name: "レイ", label: "スポーツカーマスター", color: "#ff5b67", speed: 84, handling: 58, body: "sport" },
} as const;
...
export default function Home() {
  const [tier, setTier] = useState<Tier | null>(null); const [quiz, setQuiz] = useState<Question[]>([]); const [i, setI] = useState(0); const [score, setScore] = useState(0); const [choice, setChoice] = useState<number | null>(null); const [parts, setParts] = useState({ tires:0, wing:0, ride:0, engine:0 }); const [screen,setScreen] = useState<"garage"|"quiz"|"tune"|"race">("garage");
  const car = tier ? cars[tier] : null; const earned = Math.floor(score / 5); const spent = Object.values(parts).reduce((a,b)=>a+b,0); const tokens = earned-spent; const stat = car ? {speed:car.speed+parts.engine*8+parts.tires*3+parts.wing*2, handling:car.handling+parts.tires*5+parts.ride*7+parts.wing*2} : {speed:0,handling:0};
  const start=()=>{ const t=tier!; setQuiz(pickQuiz(t));setI(0);setScore(0);setChoice(null);setParts({tires:0,wing:0,ride:0,engine:0});setScreen("quiz"); };
  const answer=(n:number)=>{if(choice!==null)return;setChoice(n);if(n===quiz[i].answer)setScore(s=>s+1)};
  const next=()=>{ if(i===quiz.length-1)setScreen("tune");else{setI(x=>x+1);setChoice(null)}};
  ...
  if(!tier) return <main className="app"><header><p className="eyebrow">お店で遊べる！</p><h1>ピットイン<br/><span>クイズレーサー</span></h1><p>クイズに答えて、きみだけの車でレースに出よう。</p></header><section className="choose"><h2>相棒の車をえらぼう</h2><div className="car-grid">{(Object.keys(cars) as Tier[]).map(t=><button key={t} className="car-card" style={{"--car":cars[t].color} as React.CSSProperties} onClick={()=>setTier(t)}><div className={`car ${cars[t].body}`}><i/><b/></div><strong>{cars[t].name}</strong><small>{cars[t].label}</small><em>{t}</em></button>)}</div></section><p className="foot">全240問から、選んだレベルに合う20問が毎回出題されます。</p></main>;
  if(screen==="garage") return <main className="app"><button className="back" onClick={()=>setTier(null)}>← 車を選び直す</button><section className="hero"><div className={`car ${car!.body}`} style={{"--car":car!.color} as React.CSSProperties}><i/><b/></div><div><p className="eyebrow">{car!.label}</p><h1>{car!.name}と<br/>クイズレース！</h1><p>20問に挑戦。5問正解ごとに、改造チケットを1枚もらえるよ。</p><button className="primary" onClick={start}>クイズをはじめる →</button></div></section><section className="how"><div>① 4択クイズ</div><div>② 車を改造</div><div>③ 5台レース</div></section></main>;
  if(screen==="quiz"){const q=quiz[i]; const ok=choice===q.answer;return <main className="app quiz"><div className="topbar"><span>{q.category}・{tier}</span><b>{i+1} / 20</b><span>正解 {score}問</span></div><div className="progress"><i style={{width:`${((i+(choice!==null?1:0))/20)*100}%`}}/></div><section className="question"><p className="eyebrow">QUESTION {String(i+1).padStart(2,"0")}</p><h2>{q.question}</h2><div className="choices">{q.choices.map((c,n)=><button key={c} className={choice===null?"":n===q.answer?"right":n===choice?"wrong":""} onClick={()=>answer(n)}><b>{["A","B","C","D"][n]}</b>{c}</button>)}</div>{choice!==null&&<div className={`result ${ok?"good":"bad"}`}><strong>{ok?"正解！すごい！":"おしい！次で取り返そう！"}</strong><p>{q.note}</p><button className="primary" onClick={next}>{i===19?"改造ガレージへ →":"次の問題へ →"}</button></div>}</section></main>}
  if(screen==="tune") return <main className="app"><header className="tune-head"><p className="eyebrow">QUIZ CLEAR</p><h1>{score} / 20問 正解！</h1><p>改造チケット：<b className="token">{tokens}枚</b>　（5問正解ごとに1枚）</p></header><section className="tune"><div className={`car ${car!.body}`} style={{"--car":car!.color,"--low":parts.ride?"6px":"0px"} as React.CSSProperties}><i/><b/>{parts.wing>0&&<u/>}</div><div className="stats"><span>スピード <b>{Math.round(stat.speed)}</b></span><span>コーナー <b>{Math.round(stat.handling)}</b></span></div><div className="parts">{[["tires","スポーツタイヤ","コーナー +5 / 速さ +3"],["wing","リアウイング","コーナー +2 / 速さ +2"],["ride","ローダウン","コーナー +7"],["engine","チューンエンジン","速さ +8"]].map(([k,n,d])=>{const key=k as keyof typeof parts;return <button key={k} disabled={tokens<=0||parts[key]>0} onClick={()=>setParts(p=>({...p,[key]:1}))}><b>{parts[key]?"装着ずみ":"チケット1枚"}</b><strong>{n}</strong><small>{d}</small></button>})}</div><button className="primary race-btn" onClick={()=>setScreen("race")}>この車でレースへ！ →</button></section></main>;
  const rank=race.findIndex(r=>r.player)+1; return <main className="app race"><header><p className="eyebrow">FINAL RACE</p><h1>{car!.name}、全開！</h1><p>クイズと改造の結果で、レースの順位が決まったよ。</p></header><section className="track"><div className="finish">FINISH</div>{race.map((r,n)=><div key={r.name} className={`lane ${r.player?"player":""}`} style={{"--pos":`${22+n*16}%`} as React.CSSProperties}><span>{n+1}</span><div className={`mini-car ${r.player?car!.body:"sedan"}`} style={r.player?{"--car":car!.color} as React.CSSProperties:{}}/><b>{r.name}</b></div>)}</section><section className="rank-card"><p>今回の結果</p><h2>{rank}位！</h2><strong>{rank===1?"優勝！ピットのヒーローだ！":rank<=3?"表彰台入り！ナイスドライブ！":"次は改造を変えて再挑戦！"}</strong><ol>{race.map((r,n)=><li key={r.name} className={r.player?"me":""}>{n+1}位　{r.name}</li>)}</ol><button className="primary" onClick={()=>{setTier(null);setScreen("garage")}}>もう一度あそぶ</button></section></main>;
}
