...
type CarId = "stella" | "nova" | "sora" | "littleBanger";
type GarageSave = { code: string; carTier: string; partsJson: string; tickets: number; challengeTier: string };
type GarageSaveResponse = { error?: string; code?: string; save?: GarageSave };

const cars = {
  stella: { name: "ステラ", label: "軽自動車", type: "スピードスター", color: "#c9a6e0", speed: 60, handling: 82, body: "compact", strength: "軽やかな走りとコーナーでの小回りが武器。", weakness: "車体が軽い分、タフさは控えめ。" },
  nova: { name: "ノヴァ", label: "普通車", type: "オールラウンダー", color: "#2f6fed", speed: 70, handling: 68, body: "sedan", strength: "バランスの取れた安定走行。どんなコースも対応できる。", weakness: "飛び抜けた武器はない。改造の選び方がカギ。" },
  sora: { name: "ソーラ", label: "レーシングカー", type: "スピードハンター", color: "#e23b4e", speed: 86, handling: 56, body: "sport", strength: "圧倒的な最高速とストレートの加速が持ち味。", weakness: "コーナーは苦手。丁寧なブレーキングが必要。" },
  littleBanger: { name: "リトルバンガー", label: "軽トラック", type: "ワイルドチャレンジャー", color: "#f2994a", speed: 58, handling: 74, body: "truck", strength: "見た目は地味だけど、タフな車体でどんな道も走り切る。", weakness: "スピードは控えめ。一発逆転を狙うタイプ。" },
} as const;
...
function makeQuestions(): Question[] {
  ...
  const lead: Record<Tier, string> = { 初級: "肩慣らしクイズ！", 中級: "本気のチャレンジ問題。", 上級: "レース前の腕試し。" };
...
export default function Home() {
  const [carId, setCarId] = useState<CarId | null>(null); const [challengeTier, setChallengeTier] = useState<Tier>("初級"); const [quiz, setQuiz] = useState<Question[]>([]); const [i, setI] = useState(0); const [score, setScore] = useState(0); const [choice, setChoice] = useState<number | null>(null); const [parts, setParts] = useState({ tires:0, wing:0, ride:0, engine:0 }); const [tickets, setTickets] = useState(0); const [raceCount, setRaceCount] = useState(0); const [transferCode, setTransferCode] = useState(""); const [restoreCode, setRestoreCode] = useState(""); const [saveMessage, setSaveMessage] = useState(""); const [screen,setScreen] = useState<"welcome"|"select"|"garage"|"quiz"|"tune"|"race">("welcome"); const [race, setRace] = useState<{name:string;value:number;player?:boolean}[]>([]);
  const car = carId ? cars[carId] : null; const tokens = tickets; const stat = car ? {speed:car.speed+parts.engine*8+parts.tires*3+parts.wing*2, handling:car.handling+parts.tires*5+parts.ride*7+parts.wing*2} : {speed:0,handling:0};
  ...
  const startRace=()=>{ if(raceCount<3){ const player=stat.speed*.62+stat.handling*.38+Math.random()*12; const rivals=["シリウス","ヴィーナス","ポルックス","リゲル"].map((name,j)=>({name,value:68+j*4+Math.random()*19})); setRace([{name:car?.name??"あなた",value:player,player:true},...rivals].sort((a,b)=>b.value-a.value)); setRaceCount(c=>c+1);setScreen("race");} };
  const makeCode=()=>Array.from(crypto.getRandomValues(new Uint32Array(10))).map(n=>"ABCDEFGHJKLMNPQRSTUVWXYZ23456789"[n%32]).join("");
  const saveGarage=async()=>{const code=transferCode||makeCode();setSaveMessage("保存しています…");try{const response=await fetch("/api/garage-save",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({code,carTier:carId,parts,tickets,challengeTier})});const data=await response.json() as GarageSaveResponse;if(!response.ok)throw new Error(data.error);setTransferCode(code);setSaveMessage("このコードを次回来店時に入力してください。");}catch(error){setSaveMessage(error instanceof Error?error.message:"保存に失敗しました。")}};
  const restoreGarage=async()=>{const code=restoreCode.trim().toUpperCase();setSaveMessage("読み込んでいます…");try{const response=await fetch(`/api/garage-save?code=${encodeURIComponent(code)}`);const data=await response.json() as GarageSaveResponse;if(!response.ok||!data.save)throw new Error(data.error??"読み込みに失敗しました。");const saved=data.save;const savedParts=JSON.parse(saved.partsJson);setCarId(saved.carTier as CarId);setChallengeTier(saved.challengeTier as Tier);setParts(savedParts);setTickets(saved.tickets);setRaceCount(0);setTransferCode(code);setSaveMessage("");setScreen("garage");}catch(error){setSaveMessage(error instanceof Error?error.message:"読み込みに失敗しました。")}};
  ...
  if(!carId) return <main className="app select-page"><header><p className="eyebrow">CAR SELECT</p><h1>まず車を<br/>選ぼう。</h1><p>車によって、得意なコースと苦手なことが違うよ。</p></header><section className="choose"><div className="car-grid">{(Object.keys(cars) as CarId[]).map(t=><button key={t} className="car-card detailed" style={{"--car":cars[t].color} as React.CSSProperties} onClick={()=>{setCarId(t);setChallengeTier("初級");setParts({tires:0,wing:0,ride:0,engine:0});setTickets(0);setRaceCount(0);setTransferCode("");setSaveMessage("");setScreen("garage")}}><div className={`car ${cars[t].body}`} style={{"--car":cars[t].color} as React.CSSProperties}><i/><b/></div><strong>{cars[t].name}</strong><small>{cars[t].label}・タイプ：{cars[t].type}</small><p className="strength">得意：{cars[t].strength}</p><p className="weakness">注意：{cars[t].weakness}</p><em>この車を選ぶ →</em></button>)}</div></section><section className="transfer"><h2>続きからあそぶ</h2><p>前回もらった10文字の引き継ぎコードを入力してください。</p><div><input aria-label="引き継ぎコード" maxLength={10} value={restoreCode} onChange={e=>setRestoreCode(e.target.value.toUpperCase())} placeholder="例：AB2CDE3FGH"/><button className="secondary" onClick={restoreGarage}>引き継ぐ</button></div>{saveMessage&&<p className="save-message">{saveMessage}</p>}</section></main>;
  if(screen==="garage") return <main className="app"><button className="back" onClick={()=>setCarId(null)}>← 車を選び直す</button><section className="hero"><div className={`car ${car!.body}`} style={{"--car":car!.color} as React.CSSProperties}><i/><b/></div><div><p className="eyebrow">{car!.label}</p><h1>{car!.name}と<br/>クイズレース！</h1><p>挑戦するクラスを選ぼう。改造した性能は、次のクラスにも引き継がれるよ。</p><div className="class-pick">{(["初級","中級","上級"] as Tier[]).map(t=><button key={t} className={challengeTier===t?"selected":""} onClick={()=>setChallengeTier(t)}>{t}クラス</button>)}</div><button className="primary" onClick={start}>{challengeTier}クラスに挑戦 →</button></div></section><section className="how"><div>① 4択クイズ</div><div>② 車を改造</div><div>③ 好きな時にレース</div></section></main>;
