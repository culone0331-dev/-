...
export default function Home() {
  ...
  if(!tier) return <main className="app top-page"><header><h1>クイズに答えて<br/>車を育てよう。</h1></header><section className="choose"><h2>相棒の車をえらぼう</h2><div className="car-grid">{(Object.keys(cars) as Tier[]).map(t=><button key={t} className="car-card" style={{"--car":cars[t].color} as React.CSSProperties} onClick={()=>{setTier(t);setChallengeTier("初級");setParts({tires:0,wing:0,ride:0,engine:0});setTickets(0);setRaceCount(0);setTransferCode("");setSaveMessage("")}}><div className={`car ${cars[t].body}`} style={{"--car":cars[t].color} as React.CSSProperties}><i/><b/></div><strong>{cars[t].name}</strong><small>{cars[t].label}</small><em>{t}</em></button>)}</div></section><section className="transfer"><h2>続きからあそぶ</h2><p>前回もらった10文字の引き継ぎコードを入力してください。</p><div><input aria-label="引き継ぎコード" maxLength={10} value={restoreCode} onChange={e=>setRestoreCode(e.target.value.toUpperCase())} placeholder="例：AB2CDE3FGH"/><button className="secondary" onClick={restoreGarage}>引き継ぐ</button></div>{saveMessage&&<p className="save-message">{saveMessage}</p>}</section><p className="foot">車の種類に関係なく、すべてのクラスに挑戦できます。</p></main>;
