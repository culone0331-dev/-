...
export default function Home() {
  ...
  const startRace=()=>{ if(raceCount<3){ const player=stat.speed*.62+stat.handling*.38+Math.random()*12; const rivals=["ミント","サンダー","モモ","ギン"].map((name,j)=>({name,value:68+j*4+Math.random()*19})); setRace([{name:car?.name??"あなた",value:player,player:true},...rivals].sort((a,b)=>b.value-a.value)); setRaceCount(c=>c+1);setScreen("race");} };
  ...
  if(!tier) return <main className="app select-page"><header><p className="eyebrow">CAR SELECT</p><h1>まず車を<br/>選ぼう。</h1><p>車によって、得意なコースと苦手なことが違うよ。</p></header><section className="choose"><div className="car-grid">{(Object.keys(cars) as Tier[]).map(t=><button key={t} className="car-card detailed" style={{"--car":cars[t].color} as React.CSSProperties} onClick={()=>{setTier(t);setChallengeTier("初級");setParts({tires:0,wing:0,ride:0,engine:0});setTickets(0);setRaceCount(0);setTransferCode("");setSaveMessage("");setScreen("garage")}}><div className={`car ${cars[t].body}`} style={{"--car":cars[t].color} as React.CSSProperties}><i/><b/></div><strong>{cars[t].name}</strong><small>{cars[t].label}</small><p className="strength">得意：{cars[t].strength}</p><p className="weakness">注意：{cars[t].weakness}</p><em>この車を選ぶ →</em></button>)}</div></section><section className="transfer"><h2>続きからあそぶ</h2><p>前回もらった10文字の引き継ぎコードを入力してください。</p><div><input aria-label="引き継ぎコード" maxLength={10} value={restoreCode} onChange={e=>setRestoreCode(e.target.value.toUpperCase())} placeholder="例：AB2CDE3FGH"/><button className="secondary" onClick={restoreGarage}>引き継ぐ</button></div>{saveMessage&&<p className="save-message">{saveMessage}</p>}</section></main>;
